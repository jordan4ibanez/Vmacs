...
function emacs.set_intern(name, value)
    return emacs.run(emacs_environment, "set", 2, { emacs.intern(name), value })
end
...
function emacs.intern(sym_name)
    return emacs.run(emacs_environment, "intern", 1, { sym_name })
end
...
function emacs.add_to_list(list, value)
    emacs.run(emacs_environment, "add-to-list", 2, { list, value })
end
...
function emacs.add_to_list_no_ret(list, value)
    emacs.run_no_return(emacs_environment, "add-to-list", 2, { list, value })
end

--- Load a feature
-- @param feature The feature to load
function emacs.require(feature)
    emacs.run_no_return(emacs_environment, "require", 1, { feature })
end

--- Load a feature interned.
-- @param feature The interned feature to load
function emacs.require_interned(feature)
    emacs.run_no_return(emacs_environment, "require", 1, { emacs.intern(feature) })
end
...
function emacs.load(file)
    return emacs.run(emacs_environment, "load", 1, { file })
end
...
function emacs.eval(form)
    return emacs.run(emacs_environment, "eval", 1, { form })
end
...
function emacs.eval_expression(form)
    return emacs.run(emacs_environment, "eval-expression", 1, { form })
end

function emacs.add_hook(hook, func, lm_local)
    if not lm_local then
        emacs.run_no_return(emacs_environment, "add-hook", 2, { hook, func })
    else
        emacs.run_no_return(emacs_environment, "add-hook", 4, { hook, func, nil, true })
    end
end

function emacs.add_hook_intern(hook, func, lm_local)
    if not lm_local then
        emacs.run_no_return(emacs_environment, "add-hook", 2, { emacs.intern(hook), func })
    else
        emacs.run_no_return(emacs_environment, "add-hook", 4, { emacs.intern(hook), func, nil, true })
    end
end

function emacs.make_local_variable(symbol)
    return emacs.run(emacs_environment, "make-local-variable", 1, { symbol })
end

function emacs.kbd(keys)
    return emacs.run(emacs_environment, "kbd", 1, { keys })
end

function emacs.define_key(keymap, key, def)
    emacs.run_no_return(emacs_environment, "define-key", 3, { keymap, key, def })
end
